...
export class EmailService {
     ...
     */
    public async sendInvitationEmail(invitation: InvitationEmailData): Promise<boolean> {
...
<body>
     ...
     */
    private getInvitationEmailText(data: {
        recipientEmail: string;
        senderName: string;
        organizationName: string;
        role: string;
...
<body>
     ...
     */
    private getWelcomeEmailText(data: {
        firstName: string;
        organizationName: string;
        dashboardLink: string;
    }): string {
...
© ${new Date().getFullYear()} ${process.env.APP_NAME || 'Tailor Bill Craft'}. All rights reserved.
     ...
     */
    public async testEmailSending(): Promise<boolean> {
        if (!this.isConfigured || !this.transporter) {
